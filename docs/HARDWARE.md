...
```
  ...
  Hall sensor GPIO24 ----> Pi                   the block's magnet
...
| Trigger | magnet in the block, sensor where the block seats on the base |
...
| Neodymium disc magnet | in the `Pull me!!! Statement` block, which it also holds to the base |
...
distance the magnet sits at when the block is on the base. Under-spec hall
...
pull-up takes it HIGH. The service reads LOW as "block on the base" and HIGH
as "block pulled off".

If Rocky talks with the block on the base and falls silent when it comes off,
...
but pulling the block off does nothing. Manual triggers and per-clip
previews keep working, which makes this the setting to use while handling the figurine
...
| Pulling the block off does nothing at all | the trigger is disarmed; check the toggle or `/api/state` |
