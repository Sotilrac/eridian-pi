...
Then the real thing, with the block on the base:

| Do this | Expect |
|---|---|
| Pull the `Pull me!!! Statement` block off the base | a clip starts |
| Leave it off | the same clip repeats, one second apart |
| Put it back on the base | silence, immediately, mid-word |
| Pull it off again | a *different* clip |
| Repeat until every clip has played | no clip repeats until all have |
| Turn off the magnet toggle, then pull it off | nothing; manual trigger still works |
...
leaves the sensor being read and reported, but pulling the block off no
longer starts anything, so the figurine can be handled or worked on in
silence.
